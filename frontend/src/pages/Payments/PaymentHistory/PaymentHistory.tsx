...
interface Payment {
  id: string; date: string; shipmentId: string; amount: number; token: string; status: PaymentStatus; txHash: string;
}

const statusClasses: Record<PaymentStatus, string> = {
  Pending:  "bg-[rgba(245,158,11,0.15)] text-[#fbbf24] border border-[rgba(245,158,11,0.3)]",
  Escrowed: "bg-[rgba(98,255,255,0.15)] text-[#62ffff] border border-[rgba(98,255,255,0.3)]",
  Released: "bg-[rgba(16,185,129,0.15)] text-[#34d399] border border-[rgba(16,185,129,0.3)]",
...
const PaymentHistory: React.FC = () => {
  ...
  const tableContainerClass = "bg-[rgba(19,186,186,0.05)] border border-[rgba(98,255,255,0.2)] rounded-2xl overflow-hidden mb-5 shadow-[inset_0_0_20px_0px_rgba(0,128,128,0.3)]";
  const thClass = "text-left px-6 py-4 text-[11px] font-semibold text-[#62ffff] uppercase border-b border-[rgba(98,255,255,0.2)]";
    ...
    return (
      <div className="p-6 md:p-4">
        ...
        <div className={`${tableContainerClass} px-10 py-20 text-center`}>
          ...
          <h2 className="text-xl font-bold mb-2 text-[#62ffff]">No Payments Found</h2>
          <p className="text-text-secondary text-sm">There are no payment transactions matching your criteria.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 md:p-4">
      ...
      <div className="flex justify-between items-start mb-6 lg:flex-col lg:gap-4">
        <div>
          <h1 className="text-2xl font-bold mb-1 sm:text-xl">Payment History</h1>
        ...
        <div className="flex gap-3 lg:w-full">
          <div className="relative flex items-center">
            <select
              value={filterStatus}
              onChange={(e) => { setFilterStatus(e.target.value as PaymentStatus | "All"); setCurrentPage(1); }}
            ...
            <ChevronDown size={16} className="absolute right-3 pointer-events-none text-text-secondary" />
          </div>
        </div>
      </div>
      ...
      <div className={`${tableContainerClass} md:overflow-x-auto`}>
        <table className="w-full border-collapse md:min-w-[800px]">
          <thead className="bg-[rgba(19,186,186,0.1)]">
            <tr>
              <th className={`${thClass} cursor-pointer select-none`} onClick={() => setSortOrder(prev => prev === "desc" ? "asc" : "desc")}>
              ...
              >
                ...
                <td className={tdClass}>
                  <div className="flex flex-col gap-0.5">
                    <span className="font-semibold text-sm">${payment.amount.toLocaleString()}</span>
                    <span className="text-[11px] text-text-secondary uppercase">{payment.token}</span>
                  </div>
                </td>
                <td className={tdClass}>
                  <span className={`px-2.5 py-1 rounded-full text-[11px] font-semibold uppercase inline-block ${statusClasses[payment.status]}`}>
                    {payment.status}
                  </span>
                </td>
                <td className={`${tdClass} font-['Courier_New',monospace] text-xs`}>
                  <a
