      ...
      {!submitted ? (
        <form className="flex flex-col gap-8" onSubmit={handleSubmit}>
          ...
          >
            ...
            {previewUrl ? (
              <div className="w-full h-full flex flex-col items-center gap-4">
                <img src={previewUrl} alt="Delivery Proof Preview" className="max-w-full max-h-[300px] rounded-lg object-contain" />
                <button
                  type="button"
              ...
              <div className="flex flex-col items-center text-center text-[rgba(255,255,255,0.7)]">
                ...
                <p className="text-[1.1rem] font-medium text-white m-0 mb-2">Click or drag & drop to upload</p>
                <p className="text-[0.9rem] m-0 text-[rgba(200,230,240,0.5)]">SVG, PNG, JPG or GIF (max. 10MB)</p>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-3">
            <label htmlFor="recipientName" className="text-white font-medium text-base">Recipient Name</label>
            <input
              type="text"
              id="recipientName"
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="bg-[rgba(0,0,0,0.2)] rounded-2xl overflow-hidden border border-[rgba(255,255,255,0.05)]">
            <div className="w-full bg-[rgba(0,0,0,0.5)] flex items-center justify-center p-8 min-h-[200px]">
              {previewUrl && <img src={previewUrl} alt="Submitted Delivery Proof" className="max-w-full max-h-[400px] rounded-lg object-contain" />}
            </div>
            <div className="px-8 py-6 bg-[rgba(255,255,255,0.02)] grid grid-cols-[auto_1fr] gap-x-8 gap-y-4 items-center md:grid-cols-1 md:gap-y-2">
              <p className="text-[rgba(255,255,255,0.5)] text-[0.9rem] m-0">Signed By</p>
              <p className="text-white text-[1.1rem] font-medium m-0 md:mb-4">{recipientName}</p>
              <p className="text-[rgba(255,255,255,0.5)] text-[0.9rem] m-0">Timestamp</p>
              <p className="text-white text-[1.1rem] font-medium m-0">{new Date().toLocaleString()}</p>
            </div>
          </div>
        </div>
      )}
